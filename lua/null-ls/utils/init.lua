...
local function flatten(arg)
    if (M.has_version("0.12")) then
        return vim.iter(arg):flatten():totable()
    else
        return vim.tbl_flatten(arg)
    end
end
...
M.path = {
    ...
    join = function(...)
        return table.concat(flatten({ ... }), path_separator):gsub(path_separator .. "+", path_separator)
    end,
}
